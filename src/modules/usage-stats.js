...
function setupMiddleware(app) {
    ...
    app.use('*', async (c, next) => {
        ...
        if (req.method === 'POST' && TRACKED_PATHS.includes(req.path)) {
            try {
                // Hono req.json() is async and can only be read once if not handled carefully
                // But we usually want the body in the handler too.
                // For stats, we might want to be less invasive or use a cloned body.
